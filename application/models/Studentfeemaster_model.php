...
class Studentfeemaster_model extends MY_Model
{
    ...
    public function getDueTransportFeeByStudent($student_session_id, $route_pickup_point_id, $date)
    {
        if ($student_session_id != NULL && $route_pickup_point_id != NULL) {
            ...
            $sql = "SELECT student_transport_fees.*,transport_feemaster.month,transport_feemaster.due_date ,transport_feemaster.fine_amount, transport_feemaster.fine_type,transport_feemaster.fine_percentage,IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` ,route_pickup_point.fees, 'transport' as fee_category FROM `student_transport_fees` INNER JOIN transport_feemaster on transport_feemaster.id =student_transport_fees.transport_feemaster_id LEFT JOIN student_fees_deposite on student_fees_deposite.student_transport_fee_id=student_transport_fees.id  INNER JOIN route_pickup_point on route_pickup_point.id = student_transport_fees.route_pickup_point_id where student_transport_fees.student_session_id=" . $student_session_id . " and student_transport_fees.route_pickup_point_id=" . $route_pickup_point_id . " and transport_feemaster.due_date < '" . $date . "' ORDER BY student_transport_fees.id asc";

            $query = $this->db->query($sql);
            $result = $query->result();

            $sql_yearly = "SELECT student_transport_yearly_fees.*,transport_yearly_feemaster.route_pickup_point_id, 'Yearly' as month,transport_yearly_feemaster.due_date ,transport_yearly_feemaster.fine_amount, transport_yearly_feemaster.fine_type,transport_yearly_feemaster.fine_percentage,IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` ,transport_yearly_feemaster.amount as fees, 'transport_yearly' as fee_category FROM `student_transport_yearly_fees` INNER JOIN transport_yearly_feemaster on transport_yearly_feemaster.id =student_transport_yearly_fees.transport_yearly_feemaster_id LEFT JOIN student_fees_deposite on student_fees_deposite.student_transport_yearly_fee_id=student_transport_yearly_fees.id  LEFT JOIN route_pickup_point on route_pickup_point.id = transport_yearly_feemaster.route_pickup_point_id where student_transport_yearly_fees.student_session_id=" . $student_session_id . " and transport_yearly_feemaster.route_pickup_point_id=" . $route_pickup_point_id . " and transport_yearly_feemaster.due_date < '" . $date . "' ORDER BY student_transport_yearly_fees.id asc";
            $query_yearly = $this->db->query($sql_yearly);
            $result_yearly = $query_yearly->result();

            return array_merge($result, $result_yearly);
        }
        return false;
    }
...
public function getFeeSessionGroupId($student_fees_master_id)
{
    ...
    public function fee_deposit_bulk($bulk_data, $fee_discounts = null)
    {
        ...
        foreach ($bulk_data as $fee_key => $fee_data) {
            ...
            if ($fee_data['fee_category'] == "fees") {
                ...
                $fee_data['student_transport_yearly_fee_id'] = NULL;
            ...
            } elseif ($fee_data['student_transport_yearly_fee_id'] > 0 && $fee_data['fee_category'] == "transport_yearly") {

                $fee_data['student_fees_master_id'] = NULL;
                $fee_data['fee_groups_feetype_id'] = NULL;
                $fee_data['student_transport_fee_id'] = NULL;
                $this->db->where('student_transport_yearly_fee_id', $fee_data['student_transport_yearly_fee_id']);
            }
        ...
        if ($this->db->trans_status() === false) {
            ...
            if (file_exists(APPPATH . 'libraries/Accounts_integration.php')) {
                ...
                foreach ($fees_return as $fee_ret) {
                    $this->accounts_integration->sync_fee($fee_ret['invoice_id']);
                }
            }
            
            // Custom Receipt Logging
            foreach ($fees_return as $fee_ret) {
                $fee_cat = isset($fee_ret['fee_category']) ? $fee_ret['fee_category'] : 'common';
                $this->generate_custom_receipt($fee_ret['invoice_id'], $fee_ret['sub_invoice_id'], $fee_cat);
            }
    ...
    public function fee_deposit($data, $send_to, $fee_discounts,$date=null)
    {
        ...
        } elseif (isset($data['student_transport_fee_id']) && $data['student_transport_fee_id'] > 0 && $data['fee_category'] == "transport") {
            $this->db->where('student_transport_fee_id', $data['student_transport_fee_id']);
        } elseif (isset($data['student_transport_yearly_fee_id']) && $data['student_transport_yearly_fee_id'] > 0 && $data['fee_category'] == "transport_yearly") {
            $this->db->where('student_transport_yearly_fee_id', $data['student_transport_yearly_fee_id']);
        }
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($this->db->trans_status() === false) {
                ...
                if (file_exists(APPPATH . 'libraries/Accounts_integration.php')) {
                    $this->load->library('accounts_integration');
                    $this->accounts_integration->sync_fee($row->id);
                }
                
                // Custom Receipt Logging
                if (isset($data['student_transport_fee_id']) && $data['student_transport_fee_id'] > 0) {
                    $fee_cat = 'transport';
                } elseif (isset($data['student_transport_yearly_fee_id']) && $data['student_transport_yearly_fee_id'] > 0) {
                    $fee_cat = 'transport_yearly';
                } else {
                    $fee_cat = 'common';
                }
                $this->generate_custom_receipt($row->id, $inv_no, $fee_cat);
            ...
            if ($this->db->trans_status() === false) {
                ...
                if (file_exists(APPPATH . 'libraries/Accounts_integration.php')) {
                    $this->load->library('accounts_integration');
                    $this->accounts_integration->sync_fee($inserted_id);
                }
                ...
                $this->generate_custom_receipt($inserted_id, 1, $fee_cat);
    ...
    {
        $this->db->select('`student_fees_deposite`.*,students.firstname,students.middlename,students.lastname,students.father_name,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,`fee_groups`.`name`, `feetype`.`type`, `feetype`.`code`,feetype.is_system,student_fees_master.student_session_id,students.admission_no')->from('student_fees_deposite');
        ...
        $module = $this->module_model->getPermissionByModulename('transport');
        if ($module['is_active']) {
            $this->db->select('`student_fees_deposite`.*,students.firstname,students.middlename,students.lastname,students.father_name,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,"Transport Fees" as name, "Transport Fees" as `type`, 0 as is_system,student_transport_fees.student_session_id,students.admission_no,transport_feemaster.month as `code')->from('student_fees_deposite');
            ...
            $this->db->select('`student_fees_deposite`.*,students.firstname,students.middlename,students.lastname,students.father_name,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,"Transport Fees" as name, "Transport Fees" as `type`, 0 as is_system,student_transport_yearly_fees.student_session_id,students.admission_no,"Yearly" as `code`')->from('student_fees_deposite');

            $this->db->join('student_transport_yearly_fees', 'student_transport_yearly_fees.id = `student_fees_deposite`.`student_transport_yearly_fee_id`');
            $this->db->join('transport_yearly_feemaster', '`student_transport_yearly_fees`.`transport_yearly_feemaster_id` = `transport_yearly_feemaster`.`id`');
            $this->db->join('student_session', 'student_session.id= `student_transport_yearly_fees`.`student_session_id`', 'INNER');
            $this->db->join('classes', 'classes.id= student_session.class_id');
            $this->db->join('sections', 'sections.id= student_session.section_id');
            $this->db->join('students', 'students.id=student_session.student_id');
            $this->db->where('student_session.session_id', $this->current_session);
            ...
            $query_yearly        = $this->db->get();
            $result_yearly = $query_yearly->result();

            $result_value1 = array_merge($result_value1, $result_yearly);
        } else {
            $result_value1 = array();
        }
        if ($feetype_id != null) {
            ...
            foreach ($result_value2 as $key => $value) {
                ...
                if (!empty($return)) {
                    foreach ($return as $r_key => $r_value) {
                        ...
                        $a['father_name']            = isset($value->father_name) ? $value->father_name : '';
        ...
        // Sort chronologically by date then by custom receipt id (true transaction order - latest on top)
        usort($return_array, function($item1, $item2) {
            $date1 = strtotime($item1['date']);
            $date2 = strtotime($item2['date']);
            if ($date1 == $date2) {
                return $item2['custom_receipt_id'] - $item1['custom_receipt_id'];
            }
            return $date2 - $date1;
        });
    ...
    {
        $sql = "SELECT student_transport_fees.*,student_session.id as student_session_id,route_pickup_point.fees,transport_feemaster.month,transport_feemaster.due_date ,transport_feemaster.fine_amount, transport_feemaster.fine_type,transport_feemaster.fine_percentage,students.id as student_id,students.firstname,students.middlename,students.admission_no,students.lastname,student_session.class_id,classes.class,sections.section,students.guardian_name,students.guardian_phone,students.father_name,student_session.section_id,student_session.student_id, IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_transport_fees` INNER JOIN transport_feemaster on transport_feemaster.id =student_transport_fees.transport_feemaster_id   LEFT JOIN student_fees_deposite on student_fees_deposite.student_transport_fee_id=student_transport_fees.id INNER JOIN student_session on student_session.id= student_transport_fees.student_session_id INNER JOIN classes on classes.id= student_session.class_id INNER JOIN sections on sections.id= student_session.section_id INNER JOIN students on students.id=student_session.student_id INNER JOIN route_pickup_point on route_pickup_point.id = student_transport_fees.route_pickup_point_id  WHERE student_transport_fees.id=" . $trans_fee_id;
        $query = $this->db->query($sql);
        return $query->row();
    }
    
    public function getTransportYearlyFeeByID($trans_fee_id) 
    {
        $sql = "SELECT student_transport_yearly_fees.*,student_session.id as student_session_id,transport_yearly_feemaster.amount as fees,'Yearly' as month,transport_yearly_feemaster.due_date ,transport_yearly_feemaster.fine_amount, transport_yearly_feemaster.fine_type,transport_yearly_feemaster.fine_percentage,students.id as student_id,students.firstname,students.middlename,students.admission_no,students.lastname,student_session.class_id,classes.class,sections.section,students.guardian_name,students.guardian_phone,students.father_name,student_session.section_id,student_session.student_id, IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail` FROM `student_transport_yearly_fees` INNER JOIN transport_yearly_feemaster on transport_yearly_feemaster.id =student_transport_yearly_fees.transport_yearly_feemaster_id   LEFT JOIN student_fees_deposite on student_fees_deposite.student_transport_yearly_fee_id=student_transport_yearly_fees.id INNER JOIN student_session on student_session.id= student_transport_yearly_fees.student_session_id INNER JOIN classes on classes.id= student_session.class_id INNER JOIN sections on sections.id= student_session.section_id INNER JOIN students on students.id=student_session.student_id LEFT JOIN route_pickup_point on route_pickup_point.id = transport_yearly_feemaster.route_pickup_point_id  WHERE student_transport_yearly_fees.id=" . $trans_fee_id;
        $query = $this->db->query($sql);
        return $query->row();
    }
    ...
    {
        $this->db->select('`student_fees_deposite`.*,students.id as std_id,students.firstname,students.middlename,students.lastname,students.admission_no,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,pickup_point.name as `pickup_name`,transport_route.route_title,transport_route.id as transport_route_id,pickup_point.id as pickup_point_id,transport_feemaster.month,transport_feemaster.due_date,transport_feemaster.fine_amount,transport_feemaster.fine_type,route_pickup_point.fees,student_transport_fees.student_session_id')->from('student_fees_deposite');
        $this->db->join('student_transport_fees', 'student_transport_fees.id = student_fees_deposite.student_transport_fee_id');
    ...
    public function getTransportYearlyFeeByInvoice($invoice_id, $sub_invoice_id)
    {
        $this->db->select('`student_fees_deposite`.*,students.id as std_id,students.firstname,students.middlename,students.lastname,students.admission_no,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,pickup_point.name as `pickup_name`,transport_route.route_title,transport_route.id as transport_route_id,pickup_point.id as pickup_point_id,"Yearly" as month,transport_yearly_feemaster.due_date,transport_yearly_feemaster.fine_amount,transport_yearly_feemaster.fine_type,transport_yearly_feemaster.amount as fees,student_transport_yearly_fees.student_session_id')->from('student_fees_deposite');
        $this->db->join('student_transport_yearly_fees', 'student_transport_yearly_fees.id = student_fees_deposite.student_transport_yearly_fee_id');
        ...
        $this->db->join('student_session', 'student_session.id= student_transport_yearly_fees.student_session_id');
        $this->db->join('classes', 'classes.id= student_session.class_id');
        $this->db->join('sections', 'sections.id= student_session.section_id');
        $this->db->join('students', 'students.id=student_session.student_id');
    ...
    {
        $sql = "SELECT student_transport_fees.*,transport_feemaster.month,transport_feemaster.due_date ,route_pickup_point.fees,transport_feemaster.fine_amount, transport_feemaster.fine_type,transport_feemaster.fine_percentage,IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail`, 'transport' as fee_category FROM `student_transport_fees` INNER JOIN transport_feemaster on transport_feemaster.id =student_transport_fees.transport_feemaster_id  LEFT JOIN student_fees_deposite on student_fees_deposite.student_transport_fee_id=student_transport_fees.id INNER JOIN route_pickup_point on route_pickup_point.id = student_transport_fees.route_pickup_point_id  where student_transport_fees.id=" . $this->db->escape($student_transport_fee_id);
        $query = $this->db->query($sql);
        return $query->row();
    }
    
    public function studentTransportYearlyDeposit($student_transport_yearly_fee_id)
    {
        $sql = "SELECT student_transport_yearly_fees.*,'Yearly' as month,transport_yearly_feemaster.due_date ,transport_yearly_feemaster.amount as fees,transport_yearly_feemaster.fine_amount, transport_yearly_feemaster.fine_type,transport_yearly_feemaster.fine_percentage,IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail`, 'transport_yearly' as fee_category FROM `student_transport_yearly_fees` INNER JOIN transport_yearly_feemaster on transport_yearly_feemaster.id =student_transport_yearly_fees.transport_yearly_feemaster_id  LEFT JOIN student_fees_deposite on student_fees_deposite.student_transport_yearly_fee_id=student_transport_yearly_fees.id LEFT JOIN route_pickup_point on route_pickup_point.id = transport_yearly_feemaster.route_pickup_point_id  where student_transport_yearly_fees.id=" . $this->db->escape($student_transport_yearly_fee_id);
        $query = $this->db->query($sql);
        return $query->row();
    } 
	...
	public function getStudentTransportFees($student_session_id, $route_pickup_point_id)
    {
        if ($student_session_id != NULL && $route_pickup_point_id != NULL) {
            ...
            $sql_yearly = "SELECT student_transport_yearly_fees.*,transport_yearly_feemaster.route_pickup_point_id, 'Yearly' as month,transport_yearly_feemaster.due_date ,transport_yearly_feemaster.amount as fees,transport_yearly_feemaster.fine_amount, transport_yearly_feemaster.fine_type,transport_yearly_feemaster.fine_percentage,IFNULL(student_fees_deposite.id,0) as `student_fees_deposite_id`, IFNULL(student_fees_deposite.amount_detail,0) as `amount_detail`, 'transport_yearly' as fee_category FROM `student_transport_yearly_fees` INNER JOIN transport_yearly_feemaster on transport_yearly_feemaster.id =student_transport_yearly_fees.transport_yearly_feemaster_id LEFT JOIN student_fees_deposite on student_fees_deposite.student_transport_yearly_fee_id=student_transport_yearly_fees.id where student_transport_yearly_fees.student_session_id=" . $student_session_id . " ORDER BY student_transport_yearly_fees.id asc";
            $query_yearly = $this->db->query($sql_yearly);
            $result_yearly = $query_yearly->result();
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $d_key => $d_value) {
                if ($d_value['fee_category'] == "transport") {
                    ...
                    $data[$d_key]['student_transport_yearly_fee_id'] = NULL;
                } elseif ($d_value['fee_category'] == "transport_yearly") {
                    $this->db->where('student_transport_yearly_fee_id', $data[$d_key]['student_transport_yearly_fee_id']);
                    $data[$d_key]['student_fees_master_id'] = NULL;
                    $data[$d_key]['fee_groups_feetype_id'] = NULL;
                ...
                if ($q->num_rows() > 0) {
                    ...
                    $collected_fees[] = array(
                        ...
                        'student_transport_fee_id' => isset($data[$d_key]['student_transport_fee_id']) ? $data[$d_key]['student_transport_fee_id'] : NULL,
                        'student_transport_yearly_fee_id' => isset($data[$d_key]['student_transport_yearly_fee_id']) ? $data[$d_key]['student_transport_yearly_fee_id'] : NULL,
    ...
    public function getOnlineFeeCollectionReport($start_date, $end_date)
    {
        ...
        if ($module['is_active']) {
            ...
            $this->db->select('`student_fees_deposite`.*,students.firstname,students.middlename,students.lastname,student_session.class_id,classes.class,sections.section,student_session.section_id,student_session.student_id,"Transport Fees" as name, "Transport Fees" as `type`, "Yearly" as `code`,0 as is_system,student_transport_yearly_fees.student_session_id,students.admission_no')->from('student_fees_deposite');
    ...
    public function generate_custom_receipt($student_fees_deposite_id, $sub_invoice_id, $fee_category) {
        $settings = $this->db->get('custom_receipt_settings')->row();
        if(!$settings) return;
        
        $is_transport = ($fee_category == 'transport' || $fee_category == 'transport_yearly');
        
        $prefix = $is_transport ? $settings->transport_prefix : $settings->common_prefix;
        $current = $is_transport ? $settings->transport_current : $settings->common_current;
        
        $receipt_no = $prefix . $current;
        
        $log_data = array(
            'student_fees_deposite_id' => $student_fees_deposite_id,
            'sub_invoice_id' => $sub_invoice_id,
            'receipt_no' => $receipt_no,
            'fee_type' => $is_transport ? 'transport' : 'common',
            'status' => 'Collected'
        );
        $this->db->insert('custom_receipt_logs', $log_data);
        
        // increment current
        $update_field = $is_transport ? 'transport_current' : 'common_current';
        $this->db->set($update_field, "$update_field + 1", FALSE);
        $this->db->where('id', 1);
        $this->db->update('custom_receipt_settings');
    }
    
    public function getFeesDashboardStats() {
        $session_id = $this->current_session;

        // 1. Total Fees (Assigned)
        $sql_total = "SELECT SUM(fee_groups_feetype.amount) as total_fees 
                      ...
                      WHERE student_session.session_id = ? AND fee_session_groups.session_id = ? AND students.is_active = 'yes'";
        $q_total = $this->db->query($sql_total, array($session_id, $session_id));
        $total_fees = 0;
        if ($q_total) {
            $res_total = $q_total->row();
            $total_fees = ($res_total && $res_total->total_fees) ? $res_total->total_fees : 0;
        }
        // Add Transport Fees to Total
        $sql_transport = "SELECT SUM(route_pickup_point.fees) as transport_fees
                          FROM student_transport_fees
                          INNER JOIN student_session ON student_session.id = student_transport_fees.student_session_id
                          INNER JOIN students ON students.id = student_session.student_id
                          INNER JOIN route_pickup_point ON route_pickup_point.id = student_transport_fees.route_pickup_point_id
                          WHERE student_session.session_id = ? AND students.is_active = 'yes'";
        $q_trans = $this->db->query($sql_transport, array($session_id));
        if ($q_trans) {
            $res_trans = $q_trans->row();
            $total_fees += ($res_trans && $res_trans->transport_fees) ? $res_trans->transport_fees : 0;
        }

        // 2. Fetch all deposits for current session
        $sql_deposits = "SELECT sfd.amount_detail, 
                                COALESCE(ss_sfm.class_id, ss_stf.class_id) as class_id,
                                COALESCE(c.class, '') as class_name,
                                COALESCE(ss_sfm.section_id, ss_stf.section_id) as section_id
                         FROM student_fees_deposite sfd
                         LEFT JOIN student_fees_master sfm ON sfm.id = sfd.student_fees_master_id
                         LEFT JOIN student_transport_fees stf ON stf.id = sfd.student_transport_fee_id
                         LEFT JOIN student_session ss_sfm ON ss_sfm.id = sfm.student_session_id
                         LEFT JOIN student_session ss_stf ON ss_stf.id = stf.student_session_id
                         LEFT JOIN classes c ON c.id = COALESCE(ss_sfm.class_id, ss_stf.class_id)
                         WHERE (ss_sfm.session_id = ? OR ss_stf.session_id = ?)";
        $q_dep = $this->db->query($sql_deposits, array($session_id, $session_id));
        $deposits = $q_dep ? $q_dep->result() : array();
        ...
        for ($i = 1; $i <= 12; $i++) {
            $monthly_collection[date('F', mktime(0, 0, 0, $i, 10))] = 0; // initialize months
        }

        $payment_modes = array(
            'Cash' => 0, 'Cheque' => 0, 'NetBanking' => 0, 
            'Upi' => 0, 'CreditCard' => 0, 'DebitCard' => 0, 
            'PaymentGateway' => 0, 'Other' => 0
        );
        ...
        foreach ($deposits as $dep) {
            ...
            if (!isset($class_wise[$class_name])) {
                $class_wise[$class_name] = 0;
            }

            if (is_string($dep->amount_detail) && is_array(json_decode($dep->amount_detail, true))) {
                $amounts = json_decode($dep->amount_detail);
                foreach ($amounts as $amt) {
                    $total_collected += $amt->amount;
                    $total_discount += $amt->amount_discount;
                    
                    if ($amt->date == $today) {
                        $todays_collection += $amt->amount;
                    }

                    $month_name = date('F', strtotime($amt->date));
                    if (isset($monthly_collection[$month_name])) {
                        $monthly_collection[$month_name] += $amt->amount;
                    } else {
                        $monthly_collection[$month_name] = $amt->amount;
                    }
        ...
        $due_pending = $total_fees - ($total_collected + $total_discount);
        if ($due_pending < 0) {
            $due_pending = 0;
        }
        ...
        return array(
            ...
            'awaiting_discount_approval' => 0, // Mocked as 0 for now based on available models
            'discount_pending_requests' => 0, // Mocked as 0
