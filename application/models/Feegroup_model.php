...
class Feegroup_model extends MY_Model
{
    ...
    public function add($data)
    {
        ...
        // Update fee_groups_classes mapping
        if (!empty($id)) {
            $this->db->where('fee_groups_id', $id);
            $this->db->delete('fee_groups_classes');
            if (!empty($class_ids)) {
                ...
                $this->db->insert_batch('fee_groups_classes', $insert_classes);
            }
        }
    ...
    public function get_mapped_classes($fee_groups_id) {
        $this->db->select('class_id')->from('fee_groups_classes')->where('fee_groups_id', $fee_groups_id);
        $query = $this->db->get();
    ...
    public function get_mapped_class_names($fee_groups_id) {
        $this->db->select('classes.class');
        $this->db->from('fee_groups_classes');
        $this->db->join('classes', 'classes.id = fee_groups_classes.class_id');
        $this->db->where('fee_groups_classes.fee_groups_id', $fee_groups_id);
        $query = $this->db->get();
        $result = $query->result_array();
        $classes = array();
        foreach ($result as $row) {
