...
class Studentcall_model extends MY_Model
{
    ...
    public function get_calls($class_id = null, $section_id = null, $date_from = null, $date_to = null, $purpose_id = null, $status = null, $follow_up_date_from = null, $follow_up_date_to = null, $assigned_to = null)
    {
        ...
        if (!empty($follow_up_date_from) && !empty($follow_up_date_to)) {
            $this->db->where('DATE((SELECT due_date FROM student_call_followups WHERE student_call_id = student_calls.id AND status NOT IN ("Completed", "Resolved") ORDER BY id ASC LIMIT 1)) >= '.$this->db->escape($follow_up_date_from), NULL, FALSE);
            $this->db->where('DATE((SELECT due_date FROM student_call_followups WHERE student_call_id = student_calls.id AND status NOT IN ("Completed", "Resolved") ORDER BY id ASC LIMIT 1)) <= '.$this->db->escape($follow_up_date_to), NULL, FALSE);
