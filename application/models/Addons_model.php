...
class Addons_model extends MY_Model
{
    ...
    public function registerCustomAddon($data)
    {
        $insert_addon = [
            ...
            'image'        => 'backend/images/addon_default.png', // Safe default
            'article_link' => '',
            'last_update'  => date('Y-m-d H:i:s')
        ];

        return $this->db->insert('addons', $insert_addon);
    }

    public function deleteCustomAddon($product_id)
    {
        $addon = $this->getByProductID($product_id);
        if ($addon) {
            $this->db->where('addon_id', $addon->id);
            $this->db->delete('addon_versions');
