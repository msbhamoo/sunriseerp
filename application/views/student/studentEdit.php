...
<?php } ?>
                                        ...
                                        <div class="col-md-3">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="is_staff_kid" id="is_staff_kid" onchange="if(this.value == '1') { $('#staff_dropdown_container_edit').show(); } else { $('#staff_dropdown_container_edit').hide(); $('#staff_id').val(''); }">
                                                    <option value="0" <?php echo set_value('is_staff_kid', isset($student['is_staff_kid']) ? $student['is_staff_kid'] : '0') == '0' ? "selected" : ""; ?>>No</option>
                                                    <option value="1" <?php echo set_value('is_staff_kid', isset($student['is_staff_kid']) ? $student['is_staff_kid'] : '0') == '1' ? "selected" : ""; ?>>Yes</option>
                                                </select>
                                                <span class="text-danger"><?php echo form_error('is_staff_kid'); ?></span>
                                            </div>
                                        </div>
                                        <div class="col-md-3" id="staff_dropdown_container_edit" style="display: <?php echo set_value('is_staff_kid', isset($student['is_staff_kid']) ? $student['is_staff_kid'] : '0') == '1' ? 'block' : 'none'; ?>;">
                                            <div class="form-group">
                                                <label for="staff_id">Staff Name</label>
                                                <select class="form-control" name="staff_id" id="staff_id">
                                                    <option value=""><?php echo $this->lang->line('select'); ?></option>
                                                    <?php foreach ($staffList as $staff) { ?>
                                                        <option value="<?php echo $staff['id']; ?>" <?php echo set_value('staff_id', isset($student['staff_id']) ? $student['staff_id'] : '') == $staff['id'] ? "selected" : ""; ?>><?php echo $staff['name'] . ' ' . $staff['surname'] . ' (' . $staff['employee_id'] . ')'; ?></option>
                                                    <?php } ?>
                                                </select>
...
<?php }?>
                                        <div class="col-md-3 pt25">
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <button type="button" style="margin-bottom: 5px;" class="btn btn-sm btn-primary mysiblings anchorbtn"><i class="fa fa-plus"></i> <?php echo $this->lang->line('add_sibling'); ?></button>
                                                </div>
                                                <div class="col-md-12">
                                                    <div id="sibling_name_container">
                                                        ...
                                                        if (!empty($sibling_ids_val) && !empty($sibling_names_val) && $sibling_ids_val !== '0') {
        ...
        ?>
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label for="exampleInputEmail1"><?php echo $this->lang->line('pickup_point'); ?></label>
                                                           <select  class="form-control" id="pickup_point" name="route_pickup_point_id" onchange="show_transport_fee()">
                                                           </select>
                                                        <span id="pickup_point_fee_display" style="color: green; font-weight: bold; display: block; margin-top: 5px;"></span>
                                                ...
                                                <div class="col-md-3">
                                                    <div class="form-group">
                                                        ...
                                                        <?php
                                                        $is_monthly_assigned = false;
                                                        foreach ($transport_fees as $key => $value) {
                                                            if ($value['student_transport_fee_id'] > 0) {
                                                                $is_monthly_assigned = true;
                                                                break;
                                                            }
                                                        }
                                                        $default_yearly = !$is_monthly_assigned;
                                                        ?>
                                                        <select class="form-control" id="transport_fee_type" name="transport_fee_type" onchange="toggleTransportMonth(); show_transport_fee();">
                                                            <option value="Yearly" <?php echo set_select('transport_fee_type', 'Yearly', $default_yearly); ?>>Yearly</option>
                                                            <option value="Monthly" <?php echo set_select('transport_fee_type', 'Monthly', !$default_yearly); ?>>Monthly</option>
                                                        </select>
                                                        <span class="text-danger"><?php echo form_error('transport_fee_type'); ?></span>
                                                    </div>
                                                </div>
                                                <div class="col-md-3" id="transport_fees_month_div">
                                                            <div class="form-group">
                            ...
                            <img id="loader" src="<?php //echo base_url('backend/images/chatloading.gif'); ?>">
                                                    ...
                                                    <h4 class="pagetitleh2">
                                                      ...
                                                      <span class="float-right bmedium total_fees_alloted">
                                                                <span id="total_fees_amount">0.00</span>
                                                                <input type="hidden" name="total_post_fees" id="total_post_fees" value="0.00">
                                                      </span>
                                                    </h4>
                                                    <div class="row around10">
                                                        <div class="col-md-12">

                                      <div class="col-md-12" id="fee_groups_container">
                                      </div>
   </div>
</div>
...
<script type="text/javascript">
    ...
    get_pickup_point(vehroute_id,route_pickup_point_id);
        ...
        function getFeeGroupsByClass(class_id) {
            if (class_id != "") {
                var base_url = '<?php echo base_url() ?>';
                // Collect existing checked items if any, though studentEdit loads them naturally on page load
                var prev_fees = [];
                $('input[name="prev_fees_group[]"]').each(function() {
                    prev_fees.push($(this).val());
                });
                
                $.ajax({
                    type: "POST",
                    url: base_url + "student/getFeeGroupsByClass_ajax",
                    data: {'class_id': class_id, 'student_session_id': '<?php echo $student['student_session_id']; ?>'},
                    success: function (data) {
                        ...
                        calculateTotalFees(); // calculate total once loaded
                    }
                });
            } else {
                $('#fee_groups_container').html("");
            }
        }
        ...
        function calculateTotalFees() {
            ...
            $('.fee_group_chk:checked').each(function() {
                total_fees += parseFloat($(this).closest('div').find('span.fee_group_total').data('amount')) || 0;
            });
            total_fees_alloted = total_fees;
            ...
            $.ajax({
                ...
                dataType: "json",
                success: function(data) {
                    $('.total_fees_alloted').html(data.amount + '<input type="hidden" name="total_post_fees" id="total_post_fees" value="' + total_fees_alloted + '">');
                }
            });
        }
        ...
        $.ajax({
                ...
                success: function(res) {
                    ...
                    $.each(res, function (index, value) {
                        ...
                        div_data += "<option  value=" + value.route_pickup_point_id + " " + sel + ">" + value.name + fee_text + "</option>";
                    });
...
<script type="text/javascript">
    ...
    var total_multi_class = "<?php echo count($this->input->post('multi_class_row') ?: (is_array($teacher_data) ? $teacher_data : [])); ?>";
...
$(document).ready(function() {
    ...
    function toggleTransportMonth() {
        var feeType = $('#transport_fee_type').val();
        if (feeType === 'Monthly') {
            $('#transport_fees_month_div').show();
        } else {
            $('#transport_fees_month_div').hide();
        }
    }

    $(document).ready(function() {
        toggleTransportMonth();
        show_transport_fee();
    });
    
    function show_transport_fee() {
        var pickup_point_id = $('#pickup_point').val();
        var class_id = $('#class_id').val();
        var transport_fee_type = $('#transport_fee_type').val();
        
        if (pickup_point_id) {
            $.ajax({
                url: baseurl + "student/get_transport_fee",
                type: "POST",
                data: {pickup_point_id: pickup_point_id, class_id: class_id, transport_fee_type: transport_fee_type},
                dataType: 'json',
                success: function(res) {
                    if (res.status == 'success' && res.amount) {
                        $('#pickup_point_fee_display').html('Fee Amount: ' + res.amount);
                    } else {
