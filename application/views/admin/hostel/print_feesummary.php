...
if (empty($feeList)) {
    echo "<div class='alert alert-danger'>Hostel Fee record not found.</div>";
    return;
}
...
foreach ($feeList as $fee) { 
    $collected = 0;
    if (!empty($fee['deposits'])) {
        foreach ($fee['deposits'] as $dep) {
            $paid = $dep['amount'] + $dep['amount_discount'];
            $collected += $paid;
            $latest_date = $dep['date'];
        }
    }
    if ($collected > 0) {
        $particulars[] = [
            'name' => strtoupper($fee['fee_group_name']),
            'amount' => $collected
        ];
    }
    $grand_total += $fee['total_amount'];
    ...
    <div class="receipt-inner">
        ...
        <div class="meta-grid">
            ...
            <div class="meta-col right">
                <div class="fees-title">HOSTEL FEES SUMMARY</div>
                <div class="meta-row"><span class="meta-label">Receipt No :</span><span class="value">SUMMARY</span></div>
                <div class="meta-row"><span class="meta-label">Date :</span><span class="value"><?php echo $latest_date ? date($this->customlib->getSchoolDateFormat(), $this->customlib->dateyyyymmddTodateformat($latest_date)) : date($this->customlib->getSchoolDateFormat()); ?></span></div>
        ...
        <table class="receipt-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><?php echo $currency_symbol . amountFormat($part['amount']); ?></td>
                </tr>
                <?php } ?>
        ...
        <div class="footer-info">
            <div class="qr-placeholder">
                <!-- Empty QR placeholder for summary -->
            </div>
            <div class="payment-mode">Payment Mode: MULTIPLE</div>
            <div class="amount-words">Amount in Words: <?php echo convert_number_to_words($grand_collected); ?> Only</div>
            <div class="total-deposit">Total Collected: <?php echo $currency_symbol . amountFormat($grand_collected); ?></div>
        </div>
