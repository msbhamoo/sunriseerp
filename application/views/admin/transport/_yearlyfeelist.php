<?php foreach ($yearlyfeelist as $yearlyfee) { ?>
    <tr>
        <td class="mailbox-name"><?php echo $yearlyfee['route_title']; ?></td>
        <td class="mailbox-name"><?php echo $yearlyfee['pickup_point_name']; ?></td>
        <td class="mailbox-name"><?php echo $yearlyfee['class_name']; ?></td>
        ...
        <td class="mailbox-date pull-right">
            <?php if ($this->rbac->hasPrivilege('transport_fees_master', 'can_delete')) { ?>
                <a href="<?php echo base_url(); ?>admin/transportyearlyfee/delete/<?php echo $yearlyfee['id']; ?>" class="btn btn-default btn-xs" data-toggle="tooltip" title="<?php echo $this->lang->line('delete'); ?>" onclick="return confirm('<?php echo $this->lang->line('delete_confirm') ?>');">
