    ...
    function edit(route_id){
        ...
        $('#modal-title').html('<?php echo $this->lang->line('edit') ?>');
         $.ajax({
                ...
                success: function(res) {
                    // Collect all IDs and make a single bulk request
                    var ids = [];
                    ...
                    if (ids.length > 0) {
                        $.ajax({
                            url: '<?php echo base_url(); ?>admin/pickuppoint/addmore_point_bulk',
                            type: "POST",
                            data: {ids: ids},
                            dataType: 'json',
                            beforeSend: function() {
                            },
                            success: function(bulkRes) {
                                $('#pickuppoint_result').html(bulkRes);
                            },
                            error: function(xhr) {
                                alert("<?php echo $this->lang->line('error_occurred_please_try_again'); ?>");
                            },
