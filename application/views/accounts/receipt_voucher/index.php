...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header ptbnull">
                        ...
                        <?php if ($this->rbac->hasPrivilege('acc_receipt_voucher', 'can_add')) { ?>
                        <div class="box-tools pull-right">
                            <button type="button" class="btn btn-primary btn-sm" onclick="toggleOffcanvas()"><i class="fa fa-plus"></i> <?php echo $this->lang->line('add_receipt_voucher'); ?></button>
                        </div>
                        <?php } ?>
                    </div>
                    <div class="box-body">
                        <div class="table-responsive mailbox-messages">
                            <table class="table table-striped table-bordered table-hover receipt-voucher-list">
                                <thead>
                                    <tr>
            ...
            <?php if ($this->rbac->hasPrivilege('acc_receipt_voucher', 'can_add') || (isset($id) && $this->rbac->hasPrivilege('acc_receipt_voucher', 'can_edit'))) { ?>
                <div class="offcanvas-right" id="voucherOffcanvas">
                    <div class="offcanvas-header">
                        <h4 class="m-0"><b><?php echo isset($id) ? $this->lang->line('edit') : $this->lang->line('add_receipt_voucher'); ?></b></h4>
                        <button type="button" class="close" onclick="toggleOffcanvas()" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    </div>
                    <form action="<?php echo isset($id) ? site_url('accounts/receiptvoucher/edit/' . $id) : site_url('accounts/receiptvoucher/index') ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                        ...
                        <div class="box-footer" style="background:#f8f9fa;">
                            ...
                            <button type="button" class="btn btn-default pull-right" style="margin-right: 5px;" onclick="<?php echo isset($id) ? "window.location.href='".site_url("accounts/receiptvoucher")."'" : "toggleOffcanvas()"; ?>"><?php echo $this->lang->line('cancel'); ?></button>
                        </div>
                    </form>
                </div>
            <?php } ?>
...
<script type="text/javascript">
    ...
    $('#voucherOverlay').click(function() {
        <?php if (isset($id)) { ?>
            window.location.href = '<?php echo site_url("accounts/receiptvoucher"); ?>';
        <?php } else { ?>
            toggleOffcanvas();
        <?php } ?>
    });
    ...
    $(document).ready(function() {
        var printUrl = '<?php echo site_url("accounts/receiptvoucher/print_voucher/" . $this->session->flashdata("print_voucher_id")); ?>';
        ...
        if (!printWin || printWin.closed || typeof printWin.closed === 'undefined') {
            ...
            var pollInterval = setInterval(function() {
                if (localStorage.getItem('pv_print_done') === '1') {
                    ...
                    if ($.fn.DataTable.isDataTable('.receipt-voucher-list')) {
                        $('.receipt-voucher-list').DataTable().ajax.reload(null, false);
                    }
                    return;
                }
                if (printWin.closed) {
    ...
    $(document).ready(function() {
        ...
        <?php if(!isset($id)) { ?>
        initDatatable('receipt-voucher-list', 'accounts/receiptvoucher/getlist', [], [], 100);
        <?php } ?>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.approve-voucher-btn', function() {
        ...
        if (confirm('Are you sure you want to approve this voucher?')) {
            $.ajax({
                url: base_url + 'accounts/receiptvoucher/approve_voucher',
                type: 'POST',
                data: {
                    voucher_id: id,
                    action: 'approve',
