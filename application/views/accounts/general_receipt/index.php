...
<div class="content-wrapper">
    ...
    <section class="content">
        <div class="row">
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header ptbnull">
                        <h3 class="box-title titlefix"><?php echo $this->lang->line('general_receipt_voucher_list'); ?></h3>
                        <?php if ($this->rbac->hasPrivilege('acc_general_receipt', 'can_add')) { ?>
                        <div class="box-tools pull-right">
                            <button type="button" class="btn btn-primary btn-sm" onclick="toggleOffcanvas()"><i class="fa fa-plus"></i> <?php echo $this->lang->line('add_general_receipt_voucher'); ?></button>
                        </div>
                        <?php } ?>
                    </div>
                    <div class="box-body">
                        <div class="table-responsive mailbox-messages">
                            <table class="table table-striped table-bordered table-hover general-receipt-list">
                                <thead>
                                    <tr>
            ...
            <?php if ($this->rbac->hasPrivilege('acc_general_receipt', 'can_add') || (isset($id) && $this->rbac->hasPrivilege('acc_general_receipt', 'can_edit'))) { ?>
                <div class="offcanvas-right" id="voucherOffcanvas">
                    <div class="offcanvas-header">
                        <h4 class="m-0"><b><?php echo isset($id) ? $this->lang->line('edit') : $this->lang->line('add_general_receipt_voucher'); ?></b></h4>
                        <button type="button" class="close" onclick="toggleOffcanvas()" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    </div>
                    <form action="<?php echo isset($id) ? site_url('accounts/generalreceipt/edit/' . $id) : site_url('accounts/generalreceipt/index') ?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
                        <div class="offcanvas-body">
                            <?php if ($this->session->flashdata('msg')) {
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label><?php echo $this->lang->line('cash_bank'); ?> (Dr)</label><small class="req"> *</small>
                                            <?php 
                                            $dr_selected = '';
                                            ...
                                            <div id="dr_ledger_balance" style="font-size:12px; font-weight:600; margin-top:4px;"></div>
                                            <span class="text-danger"><?php echo form_error('dr_ledger_id'); ?></span>
                                        </div>
                                    </div>
                                        ...
                                        <table class="table table-bordered" id="item_table">
                                            <thead>
                                                <tr style="background:#f3f4f6;">
                                                    <th><?php echo $this->lang->line('ledger_account'); ?> (Cr)</th>
                                                    <th>Category/Head</th>
                                                ...
                                                if (isset($voucher) && !empty($voucher['items'])) {
                                                    foreach ($voucher['items'] as $item) {
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td><input type="number" step="0.01" name="amount[]" class="form-control amount" value="<?php echo $item['credit_amount']; ?>" required></td>
                        ...
                        <div class="box-footer" style="background:#f8f9fa;">
                            ...
                            <button type="button" class="btn btn-default pull-right" style="margin-right: 5px;" onclick="<?php echo isset($id) ? "window.location.href='".site_url("accounts/generalreceipt")."'" : "toggleOffcanvas()"; ?>"><?php echo $this->lang->line('cancel'); ?></button>
                        </div>
                    </form>
                </div>
            <?php } ?>
...
<script type="text/javascript">
    ...
    $('#voucherOverlay').click(function() {
        <?php if (isset($id)) { ?>
            window.location.href = '<?php echo site_url("accounts/generalreceipt"); ?>';
        <?php } else { ?>
            toggleOffcanvas();
        <?php } ?>
    });
    ...
    $(document).ready(function() {
        var printUrl = '<?php echo site_url("accounts/generalreceipt/print_voucher/" . $this->session->flashdata("print_voucher_id")); ?>';
        ...
        if (!printWin || printWin.closed || typeof printWin.closed === 'undefined') {
            ...
            var pollInterval = setInterval(function() {
                if (localStorage.getItem('pv_print_done') === '1') {
                    ...
                    if ($.fn.DataTable.isDataTable('.general-receipt-list')) {
                        $('.general-receipt-list').DataTable().ajax.reload(null, false);
                    }
                    return;
                }
                if (printWin.closed) {
    ...
    $(document).ready(function() {
        if ($('#dr_ledger_id').val()) {
            fetchBalance($('#dr_ledger_id').val(), $('#dr_ledger_balance'));
        }
        ...
        $(document).on('change', '.ledger_select', function() {
            ...
            var balDiv = row.find('.cr_ledger_balance');
            fetchBalance($(this).val(), balDiv);
        });
        
        $('.ledger_select').each(function() {
            if($(this).val()) fetchBalance($(this).val(), $(this).closest('tr').find('.cr_ledger_balance'));
        });
        <?php if(!isset($id)) { ?>
        initDatatable('general-receipt-list', 'accounts/generalreceipt/getlist', [], [], 100);
        <?php } ?>
    ...
    function submitNewPaymentMode() {
        ...
        $.ajax({
            url: '<?php echo site_url("accounts/generalreceipt/quick_add_ledger_ajax"); ?>',
            type: 'POST',
            dataType: 'JSON',
            data: {
                group_id: group_id,
            ...
            success: function(res) {
                if (res.status === 'success') {
                    ...
                    $('select[name="dr_ledger_id"]').append(newOption).trigger('change');
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="addPaymentModeModalLabel">Add Receipt Mode (Cash/Bank Account)</h4>
            </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <input type="text" id="new_ledger_name" class="form-control" placeholder="e.g. Tuition Fee Income, Sale of Books">
                </div>
                <div class="form-group">
...
$(document).ready(function() {
    ...
    $(document).on('click', '.approve-voucher-btn', function() {
        ...
        if (confirm('Are you sure you want to approve this voucher?')) {
            $.ajax({
                url: base_url + 'accounts/generalreceipt/approve_voucher',
                type: 'POST',
                data: {
                    voucher_id: id,
                    action: 'approve',
