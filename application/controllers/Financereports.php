...
if (($userdata["role_id"] == 2) && ($userdata["class_teacher"] == "yes")) {
    ...
    public function collection_list()
    {
        ...
        $this->session->set_userdata('sub_menu', 'financereports/collection_list');
        $this->session->set_userdata('subsub_menu', '');
        ...
        if ($this->form_validation->run() == false && $this->input->server('REQUEST_METHOD') == "POST") {
            ...
            $data['results'] = $this->studentfeemaster_model->getFeeCollectionReport($start_date, $end_date, null, null, null, null, null);
        }

        $data['sch_setting'] = $this->sch_setting_detail;
        $this->load->view('layout/header', $data);
        $this->load->view('financereports/collection_list', $data);
        $this->load->view('layout/footer', $data);
    }

    public function class_summary_report()
    {
        if (!$this->rbac->hasPrivilege('balance_fees_report', 'can_view')) {
            access_denied();
        }
        ...
        $this->session->set_userdata('top_menu', 'fees_collection');
        $this->session->set_userdata('sub_menu', 'financereports/class_summary_report');
        $this->session->set_userdata('subsub_menu', '');
        
        $data['title']           = 'class summary report';
        $class                   = $this->class_model->get();
        $data['classlist']       = $class;
        $data['sch_setting']     = $this->sch_setting_detail;
        
        $this->form_validation->set_rules('class_id', $this->lang->line('class'), 'trim|required|xss_clean');
        // Section is optional for this report to view all sections in a class

        $summary_data = array();

        if ($this->form_validation->run() == true || $this->input->server('REQUEST_METHOD') == "GET") {
            $class_id   = $this->input->post('class_id');
            $section_id = $this->input->post('section_id');
            ...
            if (!empty($studentlist)) {
                foreach ($studentlist as $key => $eachstudent) {
                    $class_section_key = $eachstudent['class_id'] . '_' . $eachstudent['section_id'];
                    
                    if (!isset($summary_data[$class_section_key])) {
                        $summary_data[$class_section_key] = array(
                    ...
                    $summary_data[$class_section_key]['student_count']++;
                    ...
                    if (!empty($student_total_fees)) {
                        ...
                        $summary_data[$class_section_key]['totalfee'] += $totalfee;
                        $summary_data[$class_section_key]['deposit'] += $deposit;
                        $summary_data[$class_section_key]['fine'] += $fine;
                        $summary_data[$class_section_key]['discount'] += $discount;
                        $summary_data[$class_section_key]['balance'] += ($totalfee - ($deposit + $discount));
                    }
                }
            }
        }
        ...
        $this->load->view('layout/header', $data);
        $this->load->view('financereports/class_summary_report', $data);
        $this->load->view('layout/footer', $data);
    }
