...
class Student extends Admin_Controller
{
    ...
    public function ____________________________________create()
    {
        ...
        if (!empty($transport_feemaster_id) || ($transport_fee_type == 'Yearly' && $this->input->post('vehroute_id'))) {
            $this->form_validation->set_rules('vehroute_id', $this->lang->line('route_list'), 'trim|required|xss_clean');
            $this->form_validation->set_rules('route_pickup_point_id', $this->lang->line('pickup_point'), 'trim|required|xss_clean');
            if ($transport_fee_type == 'Monthly') {
                $this->form_validation->set_rules('transport_feemaster_id[]', $this->lang->line('fees_month'), 'trim|required|xss_clean');
            }
        }
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($insert) {
                ...
                $transport_fee_type = $this->input->post('transport_fee_type');
                if ($transport_fee_type == 'Monthly' && !empty($transport_feemaster_id)) {
                    $trns_data_insert = array();
                ...
                } elseif ($transport_fee_type == 'Yearly' && !empty($route_pickup_point_id) && !empty($class_id)) {
    ...
    public function edit($id)
    {
        ...
        $data['feesessiongroup_model'] = $this->feesessiongroup_model->getFeesByGroupByStudent($student['student_session_id'], $student['class_id']);
    ...
    public function getFeeGroupsByClass_ajax()
    {
        $class_id = $this->input->post('class_id');
        $student_session_id = $this->input->post('student_session_id');
        
        if (!empty($student_session_id)) {
            $feesessiongroup_model = $this->feesessiongroup_model->getFeesByGroupByStudent($student_session_id, $class_id);
        } else {
            $feesessiongroup_model = $this->feesessiongroup_model->getFeesByGroup(null, 0, $class_id);
        }
        
        $data['feesessiongroup_model'] = $feesessiongroup_model;
        $this->load->view('student/fee_groups_checkboxes', $data);
    }
    ...
    {
        if (!$this->rbac->hasPrivilege('student_list', 'can_view')) {
            access_denied();
        }

        $this->session->set_userdata('top_menu', 'Student Information');
        $this->session->set_userdata('sub_menu', 'student/studentList');
    ...
    public function preview_custom_export()
    {
        if (!$this->rbac->hasPrivilege('student_list', 'can_view')) {
            access_denied();
        }
        ...
        $columns         = $this->input->post('columns'); // array of requested column names

        if (empty($columns)) {
            echo "<p class='text-danger'>No columns selected for preview.</p>";
            return;
        }

        $raw_data = $this->student_model->getCustomExportData($class_id, $section_id, $category_id, $gender, $search_keyword);
        
        // Take only first 5 for preview
        $preview_data = array_slice($raw_data, 0, 5);

        if (empty($preview_data)) {
            echo "<p class='text-warning'>No data found for the selected filters.</p>";
            return;
        }
        ...
        foreach ($preview_data as $row) {
            $html .= "<tr>";
            foreach ($columns as $col) {
                // If it's class_section_list, format it properly, otherwise just echo
                $val = isset($row[$col]) ? $row[$col] : '';
                if ($col == 'class_section_list') {
                    $html .= "<td>" . htmlspecialchars($row['class'] . ' - ' . $row['section']) . "</td>";
                } else {
    ...
    public function export_custom_data()
    {
        if (!$this->rbac->hasPrivilege('student_list', 'can_view')) {
            access_denied();
        }
        ...
        $format          = $this->input->post('format'); // csv or excel

        if (empty($columns)) {
            $this->session->set_flashdata('msg', '<div class="alert alert-danger">Please select at least one column to export.</div>');
            redirect('student/studentList');
        }

        $raw_data = $this->student_model->getCustomExportData($class_id, $section_id, $category_id, $gender, $search_keyword);

        // Always output CSV to avoid Excel fake extension warnings
        $ext = ($format == 'excel') ? 'csv' : 'csv';
        header('Content-Type: text/csv');
        header('Content-Disposition: attachment; filename="student_export_' . date('YmdHis') . '.' . $ext . '"');
    ...
    public function ajax_student_list_custom()
    {
        if (!$this->rbac->hasPrivilege('student_list', 'can_view')) {
            access_denied();
        }
        ...
        // Handling DataTables automatic params via our custom override if necessary.
        // DataTables sends $_POST['order'], $_POST['search'] automatically,
        // but if we want to override them via our custom dropdowns, we can inject into $_POST
        $custom_sort_by  = $this->input->post('custom_sort_by');
        $custom_sort_dir = $this->input->post('custom_sort_dir');
        $search_keyword  = $this->input->post('search_keyword');
        ...
        if (!empty($custom_sort_by)) {
            $_POST['order'][0]['dir'] = strtolower($custom_sort_dir) == 'ascending' ? 'asc' : 'desc';
            
            // Map custom sort by to column index (based on the model's orderable array)
            // 'new_students.admission_no' = 0
            // 'new_students.firstname' = 1
            // 'new_students.id' = 5 (recent admission)
            if ($custom_sort_by == 'Recent Admission') {
                $_POST['order'][0]['column'] = 5;
            } else if ($custom_sort_by == 'Name') {
                $_POST['order'][0]['column'] = 1;
            } else if ($custom_sort_by == 'Scholar No') {
                $_POST['order'][0]['column'] = 0;
            }
        }

        $sch_setting = $this->sch_setting_detail;
        $resultlist = $this->student_model->searchdtStudentListCustom($class_id, $section_id, $category_id, $gender);
            ...
            foreach ($students->data as $student_key => $student) {
                ...
                if (!empty($student->class_section_list)) {
                    ...
                    foreach ($sections as $section) {
                        // Extract just the section part inside the parentheses e.g. "Nursery(A)" -> "A"
                        if (preg_match('/\((.*?)\)/', $section, $matches)) {
                            $section_name = $matches[1];
                        } else {
                            $section_name = $section;
                        }
                        $sectionListHtml .= "<li style='padding: 0px 6px 2px;'>" . htmlspecialchars($section_name) . "</li> ";
                    }
                }
                $sectionListHtml .= "</ul>";

                $action_html = "<div class='dropdown'>";
                $action_html .= "<button class='btn btn-default btn-xs dropdown-toggle' type='button' data-toggle='dropdown'> <i class='fa fa-ellipsis-v'></i></button>";
                $action_html .= "<ul class='dropdown-menu dropdown-menu-right'>";
                
                if ($this->rbac->hasPrivilege('student', 'can_view')) {
                    $action_html .= "<li><a href='" . base_url() . "student/view/" . $student->id . "' class='text-success'><i class='fa fa-user-circle-o'></i> View Profile</a></li>";
                }
                if ($this->rbac->hasPrivilege('student', 'can_edit')) {
                    $action_html .= "<li><a href='" . base_url() . "student/edit/" . $student->id . "' class='text-primary'><i class='fa fa-pencil'></i> Edit</a></li>";
                }
                if ($this->rbac->hasPrivilege('disable_student', 'can_view')) {
                    $action_html .= "<li><a href='" . base_url() . "student/disablestudent/" . $student->id . "' class='text-danger' onclick='return confirm(\"Are you sure you want to restrict this student?\");'><i class='fa fa-ban'></i> " . $this->lang->line('restrict_student') . "</a></li>";
                }
                if ($this->module_lib->hasActive('fees_collection') && $this->rbac->hasPrivilege('collect_fees', 'can_add')) {
                    $action_html .= "<li><a href='" . base_url() . "studentfee/addfee/" . $student->student_session_id . "' class='text-info'><i class='fa fa-money'></i> Fees Edit</a></li>";
                }

                // Placeholders for certificates
                $action_html .= "<li><a href='#' class='text-muted'><i class='fa fa-download'></i> Download Filled Form</a></li>";
                $action_html .= "<li><a href='#' class='text-muted'><i class='fa fa-certificate'></i> Study Certificate</a></li>";
                $action_html .= "<li><a href='#' class='text-muted'><i class='fa fa-certificate'></i> Bonafied Certificate</a></li>";
                $action_html .= "<li><a href='#' class='text-muted'><i class='fa fa-certificate'></i> Birth Certificate</a></li>";

                $action_html .= "</ul></div>";

                $row   = array();
                // S. NO. is handled by DataTables (or we can inject it, but usually DT handles it client side)
    ...
    public function dtstudentlist()
    {
        ...
        $dt_data = array();
        $fields  = $this->customfield_model->get_custom_fields('students');
    ...
    public function dashboard()
    {
        ...
        $this->session->set_userdata('top_menu', 'student_information');
        $this->session->set_userdata('sub_menu', 'student/dashboard');
    ...
    public function get_dashboard_stats()
    {
        ...
        $stats = $this->student_model->getStudentDashboardStats($date);
    ...
    public function get_transport_fee()
    {
        ...
        if ($transport_fee_type == 'Yearly' && !empty($class_id)) {
            $this->load->model('transportyearlyfee_model');
            $yearly_fees = $this->transportyearlyfee_model->getApplicableYearlyFees($class_id, $pickup_point_id);
            if (!empty($yearly_fees)) {
                $amount = $yearly_fees[0]['amount'];
            }
        } else {
            $this->db->select('fees');
            $this->db->where('id', $pickup_point_id);
            $q = $this->db->get('route_pickup_point');
            if ($q->num_rows() > 0) {
                $amount = $q->row()->fees;
            }
        }
        ...
        $currency_symbol = $this->customlib->getSchoolCurrencyFormat();
        echo json_encode(array('status' => 'success', 'amount' => $currency_symbol . $amount));
    }
}
