...
class Studentfee extends Admin_Controller
{
    ...
    {
        if (!$this->rbac->hasPrivilege('fees_dashboard', 'can_view')) {
            access_denied();
        }

        $this->session->set_userdata('top_menu', 'fees_collection');
        $this->session->set_userdata('sub_menu', 'studentfee/dashboard');
        ...
        $this->load->view('studentfee/fees_dashboard', $data);
    ...
    public function deleteFee()
    {
        ...
        if (!empty($invoice_id)) {
            ...
            // Custom Receipt Logging (Revert logic)
            $custom_log = $this->db->order_by('id', 'desc')->get_where('custom_receipt_logs', array(
                'student_fees_deposite_id' => $invoice_id,
                'sub_invoice_id' => $sub_invoice
            ))->row();

            if ($custom_log) {
                $settings = $this->db->get('custom_receipt_settings')->row();
                if ($settings) {
                    $is_transport = ($custom_log->fee_type == 'transport');
                    ...
                    $last_receipt_no = $prefix . ($current - 1);
                    
                    if ($custom_log->receipt_no === $last_receipt_no) {
                        // It was the VERY LAST generated receipt, so we can safely decrement the counter to reuse it
                ...
                // Release the original receipt number by appending -REV and mark as Reversed
                $new_receipt_no = $custom_log->receipt_no . '-REV';
                $this->db->where('id', $custom_log->id);
                $this->db->update('custom_receipt_logs', array('status' => 'Reversed', 'receipt_no' => $new_receipt_no));
            }
        }
    ...
    public function getcollectfee()
    {
        ...
        foreach ($record_array as $key => $value) {
            ...
            if ($fee_category == "transport") {
                ...
                $feeList               = $this->studentfeemaster_model->getTransportYearlyFeeByID($trans_fee_id);
                $feeList->fee_category = $fee_category;
            } else {
    ...
    public function addstudentfee()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($transport_fees_id != 0 && $fee_category == "transport") {
                ...
                $mailsms_array->code           = "";
            } elseif ($transport_fees_id != 0 && $fee_category == "transport_yearly") {
                $mailsms_array                    = new stdClass();
                $data['student_fees_master_id']   = null;
                $data['fee_groups_feetype_id']    = null;
                $data['student_transport_yearly_fee_id'] = $transport_fees_id;

                $mailsms_array                 = $this->studentfeemaster_model->getTransportYearlyFeeByID($transport_fees_id);
                $mailsms_array->fee_group_name = $this->lang->line("transport_fees");
                $mailsms_array->type           = $mailsms_array->month;
                $mailsms_array->code           = "";
            } else {
            ...
            if ($action == "print") {
                ...
                if ($transport_fees_id != 0 && $fee_category == "transport") {
                    ...
                    $fee_record = $this->studentfeemaster_model->getTransportYearlyFeeByInvoice($receipt_data->invoice_id, $receipt_data->sub_invoice_id);
                    $data['feeList']        = $fee_record;
                    $print_record = $this->load->view('print/printTransportFeesByName', $data, true);
                } else {
    ...
    public function getBalanceFee()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($fee_category == "transport") {
                ...
                $trans_fee_id         = $this->input->post('trans_fee_id');														   
                $remain_amount_object = $this->getStudentTransportYearlyFeetypeBalance($trans_fee_id);
                $remain_amount        = (float) json_decode($remain_amount_object)->balance;
                $remain_amount_fine   = json_decode($remain_amount_object)->fine_amount;
            } else {
				 $fee_groups_feetype_id  = $this->input->post('fee_groups_feetype_id');
    ...
    public function getStudentTransportYearlyFeetypeBalance($trans_fee_id)
    {
        $data = array();

        $result          = $this->studentfeemaster_model->studentTransportYearlyDeposit($trans_fee_id);
        $amount_balance  = 0;
        $amount          = 0;
        $amount_fine     = 0;
        $amount_discount = 0;
                ...
                if ($this->input->post('amount') < 0) {
                    ...
                    if ($transport_fees_id != 0) {
                        if ($fee_category == "transport_yearly") {
                            $remain_amount = $this->getStudentTransportYearlyFeetypeBalance($transport_fees_id);
                        } else {
