...
class Receiptvoucher extends MY_Addon_AccountsController
{
    ...
    {
        if (!$this->rbac->hasPrivilege('acc_receipt_voucher', 'can_view')) {
            access_denied();
        }

        $this->session->set_userdata('top_menu', 'Accounts');
        $this->session->set_userdata('sub_menu', 'accounts/receiptvoucher');

        $data['title'] = $this->lang->line('receipt_voucher');
        $data['next_voucher_no'] = $this->accvoucher_model->peekVoucherNo('receipt');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view('accounts/receipt_voucher/index', $data);
            $this->load->view('layout/footer', $data);
        } else {
            if (!$this->rbac->hasPrivilege('acc_receipt_voucher', 'can_add')) {
                access_denied();
            }
            ...
            $data_insert = array(
                'voucher_no' => $this->accvoucher_model->generateVoucherNo('receipt'),
            ...
            redirect('accounts/receiptvoucher');
        }
    }
    ...
    public function getlist()
    {
        ...
        $result = $this->accvoucher_model->getDatatableVouchers('receipt');
        ...
        if (!empty($result->data)) {
            foreach ($result->data as $value) {
                ...
                $action .= "<a href='" . site_url('accounts/receiptvoucher/print_voucher/' . $value->id) . "' class='btn btn-acc-print btn-xs' target='_blank' data-toggle='tooltip' title='Print'><i class='fa fa-print'></i></a>";
                ...
                if ($this->rbac->hasPrivilege('acc_receipt_voucher', 'can_edit')) {
                    $action .= "<a href='" . site_url('accounts/receiptvoucher/edit/' . $value->id) . "' class='btn btn-primary btn-xs' data-toggle='tooltip' title='" . $this->lang->line('edit') . "'><i class='fa fa-pencil'></i></a>";
                }
                if ($this->rbac->hasPrivilege('acc_receipt_voucher', 'can_delete')) {
                    $action .= "<a href='" . site_url('accounts/receiptvoucher/delete/' . $value->id) . "' class='btn btn-primary btn-xs' data-toggle='tooltip' title='" . $this->lang->line('delete') . "' onclick='return confirm(\"" . $this->lang->line('delete_confirm') . "\");'><i class='fa fa-remove'></i></a>";
                }
                $action .= "</div>";
