...
class Staff extends Admin_Controller
{
    ...
    {
        $staff_info = $this->staff_model->getProfile($id);
            $upload_dir = './uploads/staff_documents/' . $staff_info['id'] . '/';

            if ($staff_info['resume'] != '' && $doc=="resume") {
                $delete_file_size1 = $this->media_storage->getUploadedFileSize($staff_info['resume'], "$upload_dir");
                $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size1);
                $this->media_storage->filedelete($staff_info['resume'], "$upload_dir/");
            } 
            if ($staff_info['joining_letter'] != ''  && $doc=="joining_letter") {
                $delete_file_size2 = $this->media_storage->getUploadedFileSize($staff_info['joining_letter'], "$upload_dir");
                $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size2);
                $this->media_storage->filedelete($staff_info['joining_letter'], "$upload_dir/");
            }
            if ($staff_info['resignation_letter'] != ''  && $doc=="resignation_letter") {
                $delete_file_size3 = $this->media_storage->getUploadedFileSize($staff_info['resignation_letter'], "$upload_dir");
                $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size3);
                $this->media_storage->filedelete($staff_info['resignation_letter'], "$upload_dir/");
            }
            if ($staff_info['other_document_file'] != ''  && $doc=="other_document_file") {
                $delete_file_size4 = $this->media_storage->getUploadedFileSize($staff_info['other_document_file'], "$upload_dir");
                $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size4);
                $this->media_storage->filedelete($staff_info['other_document_file'], "$upload_dir/");
            }
    ...
    public function ajax_attendance()
    {
        ...
        $staff        = $this->staff_model->getProfile($id);
    ...
    public function create()
    {
        ...
        $storage_array = "file,first_doc,second_doc,third_doc,fourth_doc";
            ...
            try {
                ...
                if ($insert) {
                    ...
                    if (isset($_FILES["first_doc"]) && !empty($_FILES['first_doc']['name'])) {
                        ...
                        if (IsNullOrEmptyString($resume)) {
                            $total_image_failed_size += $this->media_storage->getTmpFileSize('first_doc');
                        }
                    } else {
                        $resume = "";
                    }
                    ...
                    if (isset($_FILES["second_doc"]) && !empty($_FILES['second_doc']['name'])) {
                        ...
                        if (IsNullOrEmptyString($joining_letter)) {
                            $total_image_failed_size += $this->media_storage->getTmpFileSize('second_doc');
                        }
                    } else {
                        $joining_letter = "";
                    }
                    ...
                    if (isset($_FILES["third_doc"]) && !empty($_FILES['third_doc']['name'])) {
                        ...
                        if (IsNullOrEmptyString($resignation_letter)) {
                            $total_image_failed_size += $this->media_storage->getTmpFileSize('third_doc');
                        }
                    } else {
                        $resignation_letter = "";
                    }
                    ...
                    if (isset($_FILES["fourth_doc"]) && !empty($_FILES['fourth_doc']['name'])) {
                        ...
                        if (IsNullOrEmptyString($fourth_doc)) {
                            $total_image_failed_size += $this->media_storage->getTmpFileSize('fourth_doc');
                        }
                    } else {
                        $fourth_title = "";
                        $fourth_doc   = "";
                    ...
                    $data['error_message'] = 'Employee id ' . $employee_id . ' already exists';
                    $this->load->view('layout/header', $data);
                    $this->load->view('admin/staff/staffcreate', $data);
                    $this->load->view('layout/footer', $data);
                }
	...
	public function handle_upload()
    {
        ...
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
            ...
            if ($files = @getimagesize($tmp_name)) {
    ...
    public function handle_upload_old()
    {
        $image_validate = $this->config->item('image_validate');
        $result         = $this->filetype_model->get();
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
        ...
        if ($this->form_validation->run() == false) {
            ...
            try {
                ...
                if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                    ...
                    $prev_file_size += $this->media_storage->getUploadedFileSize($staff['image'], 'uploads/staff_images');
                ...
                $session = $this->setting_model->getCurrentSession(); //current active session id 
                ...
                if (isset($_FILES["first_doc"]) && !empty($_FILES['first_doc']['name'])) {
                    $prev_file_size += $this->media_storage->getUploadedFileSize($staff['resume'], $upload_dir);
                    $resume_doc = $this->media_storage->fileupload("first_doc", $upload_dir);

                    if (!IsNullOrEmptyString($resume_doc)) {
                        $total_image_upload_size += $this->media_storage->getTmpFileSize('first_doc');
                    }
                } else {
                    $resume_doc = $resume;
                }
                ...
                if (isset($_FILES["second_doc"]) && !empty($_FILES['second_doc']['name'])) {
                    $prev_file_size += $this->media_storage->getUploadedFileSize($staff['joining_letter'], $upload_dir);
                    $joining_letter_doc = $this->media_storage->fileupload("second_doc", $upload_dir);

                    if (!IsNullOrEmptyString($joining_letter_doc)) {
                        $total_image_upload_size += $this->media_storage->getTmpFileSize('second_doc');
                    }
                } else {
                ...
                if (isset($_FILES["third_doc"]) && !empty($_FILES['third_doc']['name'])) {
                    $prev_file_size += $this->media_storage->getUploadedFileSize($staff['resignation_letter'], $upload_dir);
                    $resignation_letter_doc = $this->media_storage->fileupload("third_doc", $upload_dir);

                    if (!IsNullOrEmptyString($resignation_letter_doc)) {
                        $total_image_upload_size += $this->media_storage->getTmpFileSize('third_doc');
                    }
                } else {
                ...
                if (isset($_FILES["fourth_doc"]) && !empty($_FILES['fourth_doc']['name'])) {
                    $prev_file_size += $this->media_storage->getUploadedFileSize($staff['other_document_file'], $upload_dir);
                    $fourth_doc = $this->media_storage->fileupload("fourth_doc", $upload_dir);

                    if (!IsNullOrEmptyString($fourth_doc)) {
                        $total_image_upload_size += $this->media_storage->getTmpFileSize('fourth_doc');
                    }
                } else {
    ...
    public function delete($id)
    {
        ...
        $upload_dir = './uploads/staff_documents/' . $id . '/';
        $staff_dir = './uploads/staff_images/';

        if ($staff['image'] != '') {
            $delete_file_size = $this->media_storage->getUploadedFileSize($staff['image'], $staff_dir);
            $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);
            $this->media_storage->filedelete($staff['image'], $staff_dir);
        }


        $delete_map = [
            'resume'              => $upload_dir,
            'joining_letter'      => $upload_dir,
            'resignation_letter'  => $upload_dir,
            'other_document_name' => $upload_dir,
            'other_document_file' => $upload_dir,
        ];

        foreach ($delete_map as $field => $folder) {

            if (!empty($staff[$field])) {
                $delete_file_size = $this->media_storage->getUploadedFileSize($staff[$field], $folder);
                $this->saasvalidation->deleteResouceQuota('storage', $delete_file_size);
                $this->media_storage->filedelete($staff[$field], $folder);
            }
        }
            ...
            if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
                ...
                if ($ext == 'csv') {
                    ...
                    if (!empty($result)) {
                        ...
                        $this->form_validation->set_rules('file', 'file', "callback_validateCanAddNewResource[no_of_staff,{$total_rows}]");
