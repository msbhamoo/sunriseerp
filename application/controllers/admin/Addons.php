...
class Addons extends Admin_Controller
{
    ...
    public function handle_upload()
    {
        ...
        if (isset($_FILES["file"]) && !empty($_FILES['file']['name'])) {
            ...
            if (!$addon_detail) {
                // Auto-register: try to read addon_info.json from the uploaded ZIP
                $addon_detail = $this->autoRegisterAddon($folder_name);
                if (!$addon_detail) {
    ...
    /**
     * Register a custom addon via the modal form (AJAX).
     */
    public function register_custom()
    {
        ...
        if (empty($product_id) || empty($name) || empty($directory) || empty($description)) {
            $return_data = ['status' => 0, 'msg' => 'Please fill all required fields.'];
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(200)
                ->set_output(json_encode($return_data));
        ...
        $existing = $this->addons_model->getByProductID($product_id);
        if ($existing) {
            $return_data = ['status' => 0, 'msg' => 'An addon with this Product ID already exists.'];
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(200)
                ->set_output(json_encode($return_data));
        ...
        $result = $this->addons_model->registerCustomAddon($data);

        if ($result) {
            $return_data = ['status' => 1, 'msg' => 'Custom addon registered successfully.'];
        } else {
            $return_data = ['status' => 0, 'msg' => 'Failed to register addon. Please check the database.'];
        }
    ...
    /**
     * Delete a custom addon registration (AJAX).
     */
    public function delete_custom()
    {
        ...
        if (empty($product_id)) {
            $return_data = ['status' => 0, 'msg' => 'Product ID is required.'];
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(200)
                ->set_output(json_encode($return_data));
        ...
        $this->addons_model->deleteCustomAddon($product_id);
        $return_data = ['status' => 1, 'msg' => 'Addon registration removed successfully.'];
    ...
    /**
     * Auto-register an addon by reading addon_info.json from the uploaded ZIP.
     * This allows custom addons to be uploaded without pre-registering in the addons table.
     */
    private function autoRegisterAddon($folder_name)
    {
        ...
        // Try to read addon_info.json from the ZIP root
        $json_content = $zip->getFromName('addon_info.json');
        if ($json_content === false) {
            // Also try with folder prefix
            $json_content = $zip->getFromName($folder_name . '/addon_info.json');
        }
        $zip->close();
        ...
        $addon_info = json_decode($json_content, true);
        if (empty($addon_info) || empty($addon_info['product_id']) || empty($addon_info['directory'])) {
            return false;
        }

        // Check if product_id already exists
        $existing = $this->addons_model->getByProductID($addon_info['product_id']);
        if ($existing) {
            return $existing;
        }

        // Insert new addon record
        $insert_data = array(
            'product_id'    => $addon_info['product_id'],
            'image'         => isset($addon_info['image']) ? $addon_info['image'] : '',
            'name'          => isset($addon_info['name']) ? $addon_info['name'] : $folder_name,
            'config_name'   => isset($addon_info['config_name']) ? $addon_info['config_name'] : $folder_name . '-config',
            'short_name'    => isset($addon_info['short_name']) ? $addon_info['short_name'] : substr($folder_name, 0, 10),
            'directory'     => $addon_info['directory'],
            'description'   => isset($addon_info['description']) ? $addon_info['description'] : '',
            'price'         => 0.00,
            'current_version' => null,
            'article_link'  => isset($addon_info['article_link']) ? $addon_info['article_link'] : '',
            'current_stage' => 0,
            'product_order' => 99,
        ...
        $this->db->insert('addons', $insert_data);

        if ($this->db->affected_rows() > 0) {
            logMessage("Auto-registered new addon: " . $addon_info['name'] . " (directory: " . $addon_info['directory'] . ")");
            return $this->addons_model->getAddonByFileName($folder_name);
        }
