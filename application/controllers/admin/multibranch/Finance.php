...
class Finance extends MY_Addon_MBController
{
    ...
    */
    ...
    /*
    This function is used to load trial balance report
    */
    public function trialbalance()
    {
        if (!$this->rbac->hasPrivilege('multi_branch_accounting_report', 'can_view')) {
            access_denied();
        }
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('subsub_menu', 'finance/trialbalance');
        $this->load->model("multibranch/multi_accreport_model");
        $data = array();
        $data['title'] = 'Consolidated Trial Balance';
        
        $to_date = date('Y-m-d');
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            $to_date = $this->customlib->dateFormatToYYYYMMDD($this->input->post('to_date'));
            $data['search_date'] = $this->input->post('to_date');
        } else {
            $data['search_date'] = date($this->customlib->getSchoolDateFormat());
        }

        $data['results'] = $this->multi_accreport_model->getConsolidatedTrialBalance($to_date);

        $this->load->view('layout/header', $data);
        $this->load->view('admin/multibranch/finance/trialbalance', $data);
        $this->load->view('layout/footer', $data);
    }

    /*
    This function is used to load profit loss report
    */
    public function profitloss()
    {
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('subsub_menu', 'finance/profitloss');
        $this->load->model("multibranch/multi_accreport_model");
        $data = array();
        $data['title'] = 'Consolidated Profit & Loss';
        
        $to_date = date('Y-m-d');
        $from_date = date('Y-m-d', strtotime("-1 year")); // For standard date picker defaults
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            $to_date = $this->customlib->dateFormatToYYYYMMDD($this->input->post('to_date'));
            $from_date = $this->customlib->dateFormatToYYYYMMDD($this->input->post('from_date'));
            $data['search_to_date'] = $this->input->post('to_date');
            $data['search_from_date'] = $this->input->post('from_date');
        } else {
            $data['search_to_date'] = date($this->customlib->getSchoolDateFormat());
            $data['search_from_date'] = date($this->customlib->getSchoolDateFormat(), strtotime("-1 year"));
        }

        $data['results'] = $this->multi_accreport_model->getConsolidatedProfitLoss($from_date, $to_date);

        $this->load->view('layout/header', $data);
        $this->load->view('admin/multibranch/finance/profitloss', $data);
        $this->load->view('layout/footer', $data);
    }

    /*
    This function is used to load balance sheet
    */
    public function balancesheet()
    {
        ...
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('subsub_menu', 'finance/balancesheet');
        $this->load->model("multibranch/multi_accreport_model");
        $data = array();
        $data['title'] = 'Consolidated Balance Sheet';
        ...
        $data['results'] = $this->multi_accreport_model->getConsolidatedBalanceSheet($to_date);

        $this->load->view('layout/header', $data);
        $this->load->view('admin/multibranch/finance/balancesheet', $data);
        $this->load->view('layout/footer', $data);
    }
}