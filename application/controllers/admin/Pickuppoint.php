...
class Pickuppoint extends Admin_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model(array("pickuppoint_model", "routepickuppoint_model", "transportyearlyfee_model"));
    ...
    public function add_student_fees()
    {
        ...
        $prev_ids              = array_filter($this->input->post('prev_ids') ?? array());
        $not_deleted           = array();
        ...
        // Handle yearly fees
        $transport_yearly_fees = $this->input->post('transport_yearly_fee');
        $prev_yearly_ids       = array_filter($this->input->post('prev_yearly_ids') ?? array());
        $not_deleted_yearly    = array();

        if (isset($_POST['transport_yearly_fee'])) {
            foreach ($transport_yearly_fees as $transport_key => $transport_value) {
                $is_prev_inserted = $this->input->post('student_transport_yearly_fee_id_' . $transport_value);
                if ($is_prev_inserted == "") {
                    $assign_data = array(
                        'student_session_id' => $student_session_id,
                        'transport_yearly_feemaster_id' => $transport_value,
                        'is_active' => 'yes'
                    );
                    $this->transportyearlyfee_model->assignStudent($assign_data);
                } else {
                    $not_deleted_yearly[] = $is_prev_inserted;
                }
            }
        }
        $remove_yearly_ids = array_diff($prev_yearly_ids, $not_deleted_yearly);
        if (!empty($remove_yearly_ids)) {
            foreach ($remove_yearly_ids as $r_id) {
                // Remove assignment where student_transport_yearly_fee_id = $r_id
    ...
    public function student_transport_months()
    {
        ...
        $data['yearly_fees'] = $this->transportyearlyfee_model->getStudentTransportYearlyFees($student_session_id, $route_pickup_point_id, $student['class_id']);
    ...
    public function addmore_point_bulk()
    {
        ...
        if (!empty($ids) && is_array($ids)) {
            foreach ($ids as $id) {
                ...
                $data['listpickup_point'] = $listpickup_point;
                $data['result'] = $pickup_pointdata;
                $data['delete_string'] = $id;
                $html .= $this->load->view("admin/pickuppoint/_addpickuppoint", $data, true);
            }
        }
